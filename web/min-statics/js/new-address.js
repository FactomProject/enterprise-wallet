...
$('#nickname-input').addClass("input-group-error");return}
selected=$("#generate-source option:selected").val()
if(selected=="import-address"){sec=$("#private-key-input").val()
if(!(sec.startsWith("Fs")||sec.startsWith("Es"))){SetError("Not a valid private key. Should start with 'Fs' for a factoid address or 'Es' for an entry credit address")
$("#private-key-input-container").addClass("input-group-error");return}
postRequest("is-valid-address",sec,function(resp){if(resp=="false"){SetError("Not a valid private key.")
$("#private-key-input-container").addClass("input-group-error");return}else{var newAddressObj={Name:Name,Secret:sec}
j=JSON.stringify(newAddressObj)
postRequest("new-address",j,function(resp){obj=JSON.parse(resp)
if(obj.Error=="none"){SetSuccess(obj)}else{SetError(obj.Error)}})}})}else if(selected=="random-ec"){postRequest("generate-new-address-ec",Name,function(resp){obj=JSON.parse(resp)
if(obj.Error=="none"){SetSuccess(obj)}else{SetError(obj.Error)}})}else if(selected=="random-factoid"){postRequest("generate-new-address-factoid",Name,function(resp){obj=JSON.parse(resp)
if(obj.Error=="none"){SetSuccess(obj)}else{SetError(obj.Error)}})}else if(selected=="new-external-address"){pub=$("#private-key-input").val()
if(!(pub.startsWith("FA")||pub.startsWith("EC"))){SetError("Not a valid public key. Should start with 'FA' for a factoid address or 'EC' for an entry credit address."+" This option adds an address to your external addresses for easier use.")
$("#private-key-input-container").addClass("input-group-error");return}
postRequest("is-valid-address",pub,function(resp){if(resp=="false"){SetError("Not a valid public key.")
...
function SetSuccess(obj){$("#error-zone").slideUp(100)
